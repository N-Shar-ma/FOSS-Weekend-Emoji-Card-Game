...
function addCardsOneByOne(timeInterval) {
  ...
  let repeater = setInterval(() => {
    if (i === maxCardsPlayable) clearInterval(repeater);
    else {
      addCard();
      i++;
    }
...
function getCorrespondingCardObject(name) {
  return (
    currentCardObjectsDeck.find((cardObject) => cardObject.name === name) ||
    wrongCardObjectsDeck.find((cardObject) => cardObject.name === name)
  );
}
...
function setGhostImage(e) {
  ...
  else cardFace = cardToGhost.getElementsByClassName("front-face")[0];
...
function moveCard() {
  ...
  if (cardObjectsDeck[0]) setTimeout(addCard, 100);
  if (currentCardObjectsDeck[0]) updateCardDropPosition();
}
...
function handleCardInDestination(card) {
  ...
  if (showName) {
    ...
    setTimeout(() => {
      nameShower.classList.remove("show");
    }, 3200);
  }
  if (isCorrectMove(innerCard)) {
    ...
    wrongCardObjectsDeck.push(
      getCorrespondingCardObject(innerCard.dataset.name)
    );
    checkLoss();
  }
}
...
function isCorrectMove(card) {
  ...
  if (cardObject[matcher] === cardDropPosition.innerText) return true;
  return false;
}
...
Your score is ${scoreCount} and you took ${hintCount} hint${
      hintCount === 1 ? "" : "s"
    }.
...
function removeFromCurrentDeck(card) {
  const index = currentCardObjectsDeck.findIndex(
    (cardObject) => card.dataset.name === cardObject.name
  );
